...
## [v4.0.0](https://github.com/sisong/HDiffPatch/tree/v4.0.0) - 2021-06-13
### Added
* cmdline add option "-SD", to create single compressed diffData, for optimize   
decompress buffer when patch, and support step by step patching when step by step downloading;   
it's better for IoT!  NOTE: old patcher can't work with this new format diffData. 
* the added create_single_compressed_diff()&patch_single_stream() can be used;
* add create_single_compressed_diff_stream(), same as create_single_compressed_diff(), but can   
control memory requires and run speed by different kMatchBlockSize value;
* now, zstd plugin default supported in cmdline;
### Changed
...
diffData saved as single compressed stream, for optimize decompress buffer when patch;
### Removed
